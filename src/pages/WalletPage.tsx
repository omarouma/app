...
const depositMethods = [
  ...
  { icon: Banknote, label: 'Bank Transfer', color: 'bg-primary', currency: 'USD' as CurrencyCode },
...
const ALL_CURRENCIES: { code: CurrencyCode; label: string; color: string }[] = [
  { code: 'GAGA', label: 'Gaga Coins', color: 'text-primary' },
  ...
  return (
    <div className="min-h-[100dvh] bg-secondary">
      ...
      <div className="bg-gradient-to-br from-primary to-[#00A300] text-white">
        ...
        <div className="px-4 pb-6">
          ...
          <div className="flex gap-2 mt-4">
            {[
              { icon: Plus, label: 'Deposit', color: 'bg-card text-primary', action: () => setShowDeposit(true) },
      ...
      <div className="p-4 space-y-4 pb-nav">
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-foreground text-sm font-semibold">Gaga Coins Value</p>
                <p className="text-muted-foreground text-[10px]">Your total portfolio value</p>
              </div>
            </div>
            <p className="text-primary font-bold text-lg">{Math.round(totalGagaValue).toLocaleString()} GAGA</p>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-secondary rounded-xl p-2.5 text-center">
              <p className="text-primary font-bold text-sm">{coins.toLocaleString()}</p>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-foreground text-sm font-semibold">{tier.label} Tier</p>
                <p className="text-muted-foreground text-[10px]">{apy}% APY staking reward</p>
              </div>
            </div>
          ...
          <div className="w-full h-2 bg-secondary rounded-full overflow-hidden">
            <motion.div
              ...
              className="h-full bg-gradient-to-r from-primary to-[#FFD700] rounded-full"
            />
          </div>
          <div className="flex justify-between mt-1.5">
            {STAKING_TIERS.filter(t => t.minCoins > 0).map(t => (
              <span key={t.label} className={`text-[9px] ${coins >= t.minCoins ? 'text-primary font-medium' : 'text-muted-foreground'}`}>
                {t.label}
              </span>
            ))}
          </div>
        ...
        >
          {[
            { icon: ArrowRightLeft, label: 'Convert', color: 'text-primary', action: () => setShowConvert(true) },
            { icon: TrendingDown, label: 'Withdraw', color: 'text-destructive', action: () => setShowWithdraw(true) },
            { icon: Banknote, label: 'Deposit', color: 'text-[#2196F3]', action: () => setShowDeposit(true) },
            { icon: Send, label: 'Send', color: 'text-primary', action: () => setShowSendToFriend(true) },
            ...
            { icon: Shield, label: 'Security', color: 'text-primary', action: () => setShowSecurity(true) },
          ].map((item) => (
            <button type="button" key={item.label}
              onClick={item.action}
              className="flex flex-col items-center gap-1.5 p-3 bg-card rounded-xl border border-border active:scale-95 transition-transform"
            >
              <item.icon size={20} className={item.color} />
              <span className="text-foreground text-[10px] font-medium">{item.label}</span>
            </button>
          ))}
        </motion.div>
        ...
        <motion.button
          ...
          className="w-full flex items-center justify-between p-4 bg-card rounded-2xl border border-border active:bg-gray-50 transition-colors"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-foreground text-sm font-medium">Redeem Promo Code</span>
              <p className="text-muted-foreground text-[10px]">Get free Gaga Coins</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
              {hasPinSet() ? <Lock size={18} className="text-primary" /> : <Unlock size={18} className="text-destructive" />}
            </div>
            <div>
              <span className="text-foreground text-sm font-medium">Wallet Security</span>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}>
          <h3 className="text-muted-foreground text-sm font-medium mb-3 flex items-center gap-2 px-1">
          ...
          <div className="space-y-2">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isPositive
                      ? 'bg-gradient-to-br from-primary/20 to-primary/30' : tx.type === 'convert'
                  ...
                  <div className="text-right">
                    <span className={`text-sm font-bold ${isPositive
                      ? 'text-primary' : tx.type === 'convert'
                        ? 'text-[#2196F3]' : 'text-destructive'
                      }`}>
                      {isPositive ? '+' : '-'}
                    ...
                    {tx.id && (
                      <p className="text-muted-foreground text-[9px] mt-0.5 font-mono">{tx.id.slice(-8).toUpperCase()}</p>
                    )}
                  </div>
                </motion.div>
              );
            ...
            {(!wallet?.transactions || wallet.transactions.length === 0) && (
              <div className="text-center py-10 bg-card rounded-xl border border-dashed border-border">
                ...
                <p className="text-xs text-muted-foreground mt-1">Start depositing to see your history</p>
              </div>
            )}
          </div>
        </motion.div>
      ...
      <Dialog open={showPromo} onOpenChange={setShowPromo}>
        <DialogContent className="bg-card border-border text-foreground sm:max-w-md">
          <DialogHeader><DialogTitle className="text-foreground flex items-center gap-2"><Gift size={18} className="text-[#FF9800]" /> Redeem Promo Code</DialogTitle></DialogHeader>
          <div className="pt-4">
            <input
              value={promoInput}
              onChange={e => { setPromoInput(e.target.value); setPromoError(''); setPromoSuccess(''); }}
              placeholder="Enter promo code"
              className="w-full bg-secondary border border-border rounded-xl px-4 py-3 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary uppercase placeholder:normal-case placeholder:text-muted-foreground"
            />
            {promoError && <p className="text-destructive text-xs mt-2">{promoError}</p>}
            {promoSuccess && <p className="text-primary text-xs mt-2">{promoSuccess}</p>}
            <button type="button" onClick={handleRedeem}
              className="w-full bg-primary hover:bg-[#00A300] text-white rounded-xl py-3 mt-4 text-sm font-bold transition-colors"
            >
              Redeem
            </button>
            <div className="mt-4 space-y-2">
              <p className="text-muted-foreground text-xs">Available codes:</p>
              {Object.entries(PROMO_CODES).map(([code, data]) => (
                <div key={code} className="flex items-center justify-between bg-secondary rounded-lg px-3 py-2">
                  <span className="text-foreground text-xs font-bold">{code}</span>
                  <span className="text-primary text-xs font-medium">+{data.coins} GAGA</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <Dialog open={showDeposit} onOpenChange={setShowDeposit}>
        <DialogContent className="bg-card border-border text-foreground sm:max-w-md">
          <DialogHeader><DialogTitle className="text-foreground flex items-center gap-2"><Banknote size={18} className="text-[#2196F3]" /> Deposit</DialogTitle></DialogHeader>
          <div className="pt-4 space-y-4">
            <div>
              <label className="text-muted-foreground text-xs mb-2 block">Select Currency</label>
              <div className="flex gap-2">
                {(['GAGA', 'USD'] as CurrencyCode[]).map(c => (
                  <button type="button" key={c}
                    onClick={() => setDepositCurrency(c)}
                    className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${depositCurrency === c ? 'bg-primary text-white' : 'bg-secondary text-muted-foreground'
                      }`}
                  >
                    {c}
                  </button>
            ...
            <div>
              <label className="text-muted-foreground text-xs mb-2 block">Select Method</label>
              <div className="grid grid-cols-2 gap-2">
                {depositMethods.filter(m => m.currency === depositCurrency || depositCurrency === 'GAGA').map((method) => (
                  ...
                  >
                    ...
                    <span className="text-foreground text-xs font-medium">{method.label}</span>
                  </button>
                ))}
              </div>
            </div>
            <div>
              <label className="text-muted-foreground text-xs mb-1 block">Amount ({depositCurrency})</label>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground font-bold">{getCurrencySymbol(depositCurrency)}</span>
                <input
                  type="number"
                  value={depositAmount}
                  onChange={e => setDepositAmount(e.target.value)}
                  placeholder="0.00"
                  className="w-full bg-secondary rounded-xl pl-10 pr-4 py-3 text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
                />
              </div>
            </div>
      ...
      <Dialog open={showWithdraw} onOpenChange={setShowWithdraw}>
        <DialogContent className="bg-card border-border text-foreground sm:max-w-md">
          <DialogHeader><DialogTitle className="text-foreground flex items-center gap-2"><TrendingDown size={18} className="text-destructive" /> Withdraw</DialogTitle></DialogHeader>
          <div className="pt-4 space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                {(['GAGA', 'USD'] as CurrencyCode[]).map(c => (
                  <button type="button" key={c}
                    ...
                    className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${withdrawCurrency === c ? 'bg-primary text-white' : 'bg-secondary text-muted-foreground'
                      }`}
                  >
                    {c}
                  </button>
            ...
            <div>
              ...
              <input
                ...
                placeholder="0.00"
                className="w-full bg-secondary rounded-xl px-4 py-3 text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
            <div>
              <label className="text-muted-foreground text-xs mb-1 block">Method (e.g. PayPal, Bank)</label>
              <input
            ...
            <div>
              <label className="text-muted-foreground text-xs mb-1 block">Account Number</label>
              <input
            ...
            <button type="button" onClick={handleWithdraw}
              ...
              className="w-full bg-destructive hover:bg-[#D32F2F] text-white rounded-xl py-3 text-sm font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
      ...
      <Dialog open={showConvert} onOpenChange={setShowConvert}>
        <DialogContent className="bg-card border-border text-foreground sm:max-w-md">
          <DialogHeader><DialogTitle className="text-foreground flex items-center gap-2"><ArrowRightLeft size={18} className="text-primary" /> Convert Currency</DialogTitle></DialogHeader>
          <div className="pt-4 space-y-4">
            <div className="flex border-b border-border overflow-x-auto">
              {(['GAGA_USD', 'USD_GAGA'] as const).map(pair => (
                <button type="button" key={pair}
                  onClick={() => setConvertTab(pair)}
                  className={`px-3 py-2 text-xs font-medium whitespace-nowrap transition-colors ${convertTab === pair ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground'
                    }`}
                >
                  {pair.replace('_', ' → ')}
                </button>
            ...
            <button type="button" onClick={handleConvert}
              ...
              className="w-full bg-primary hover:bg-[#00A300] text-white rounded-xl py-3 text-sm font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
      ...
      <Dialog open={showInterest} onOpenChange={setShowInterest}>
        <DialogContent className="bg-card border-border text-foreground sm:max-w-md">
          <DialogHeader><DialogTitle className="text-foreground flex items-center gap-2"><Sparkles size={18} className="text-[#FFD700]" /> Gaga Staking Rewards</DialogTitle></DialogHeader>
          <div className="pt-4 space-y-4">
            {interestClaimed > 0 && (
              <motion.div
              ...
              >
                <p className="text-primary font-bold text-lg">+{interestClaimed} GAGA</p>
                <p className="text-primary text-xs">Claimed successfully!</p>
              </motion.div>
            )}

            <div className="bg-secondary rounded-2xl p-4 text-center">
              <p className="text-muted-foreground text-xs mb-1">Current Balance</p>
              <p className="text-2xl font-bold text-foreground">{coins.toLocaleString()} GAGA</p>
              <p className="text-primary text-sm font-medium mt-1">{apy}% APY — {tier.label} Tier</p>
            </div>

            <div className="bg-gradient-to-r from-[#FFD700]/10 to-[#FFA500]/10 rounded-xl p-4">
              <p className="text-foreground text-sm font-medium mb-2">Available to claim</p>
              <div className="flex items-center justify-between">
                <p className="text-2xl font-bold text-foreground">+{dailyInterest} GAGA</p>
                <button type="button" onClick={handleClaimInterest}
                  disabled={interestLoading || dailyInterest <= 0}
                  className="px-4 py-2 bg-primary text-white rounded-xl text-sm font-bold disabled:opacity-50 active:bg-[#00A300] transition-colors"
                >
                  {interestLoading ? <Loader size={14} className="animate-spin" /> : 'Claim'}
                </button>
              </div>
              <p className="text-muted-foreground text-[10px] mt-1">Resets every 24 hours</p>
            </div>

            {/* Tier Table */}
            <div className="space-y-2">
              <p className="text-foreground text-sm font-medium">Staking Tiers</p>
              {STAKING_TIERS.filter(t => t.minCoins > 0).map(t => (
                <div
                  key={t.label}
                  className={`flex items-center justify-between p-2.5 rounded-lg ${coins >= t.minCoins ? 'bg-primary/10 border border-primary/30' : 'bg-secondary'
                    }`}
                >
                  <div className="flex items-center gap-2">
                    <Award size={14} className={coins >= t.minCoins ? 'text-primary' : 'text-muted-foreground'} />
                    <span className={`text-sm ${coins >= t.minCoins ? 'text-primary font-medium' : 'text-muted-foreground'}`}>{t.label}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-sm font-medium">{t.apy}%</span>
                    <span className="text-muted-foreground text-[10px] ml-2">{t.minCoins.toLocaleString()}+ GAGA</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Dialog open={showSecurity} onOpenChange={setShowSecurity}>
        <DialogContent className="bg-card border-border text-foreground sm:max-w-md">
          <DialogHeader><DialogTitle className="text-foreground flex items-center gap-2"><Shield size={18} className="text-primary" /> Wallet Security</DialogTitle></DialogHeader>
          <div className="pt-4 space-y-4">
            <div className="bg-secondary rounded-2xl p-4 text-center">
              <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-2">
                {hasPinSet() ? <Lock size={24} className="text-primary" /> : <Unlock size={24} className="text-destructive" />}
              </div>
            ...
            {!hasPinSet() ? (
              <button type="button" onClick={() => { setShowSecurity(false); setPinMode('set'); }}
                className="w-full bg-primary text-white rounded-xl py-3 text-sm font-bold hover:bg-[#00A300] transition-colors"
              >
                Set Up PIN
              </button>
            ) : (
              <>
                <button type="button" onClick={() => { setShowSecurity(false); setPinMode('set'); }}
                  className="w-full bg-secondary text-foreground rounded-xl py-3 text-sm font-bold hover:bg-secondary transition-colors"
                >
                  Change PIN
                </button>
                <button type="button" onClick={() => { clearWalletPin(user?.id || ''); setShowSecurity(false); }}
                  className="w-full text-destructive rounded-xl py-3 text-sm font-medium hover:bg-destructive/10 transition-colors"
                >
