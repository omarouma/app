  ...
  return (
    <div className="min-h-[100vh] bg-muted">
      ...
      <div className="bg-gradient-to-br from-[#00C300] to-[#00A300] text-white">
        ...
        <div className="px-4 pb-6">
          ...
          <div className="flex gap-2 mt-4">
            {[
              { icon: Plus, label: 'Deposit', color: 'bg-background text-[#00C300]', action: () => setShowDeposit(true) },
      ...
      <div className="p-4 space-y-4 pb-nav">
        ...
        >
          ...
          <div className="flex justify-between mt-1.5">
            {STAKING_TIERS.filter(t => t.minCoins > 0).map(t => (
              <span key={t.label} className={`text-[9px] ${coins >= t.minCoins ? 'text-[#00C300] font-medium' : 'text-muted-foreground'}`}>
                {t.label}
              </span>
            ))}
          </div>
        ...
        >
          {[
            ...
            <button type="button" key={item.label}
              onClick={item.action}
              className="flex flex-col items-center gap-1.5 p-3 bg-background rounded-xl border border-border active:scale-95 transition-transform"
            >
        ...
        <motion.button
          ...
          className="w-full flex items-center justify-between p-4 bg-background rounded-2xl border border-border active:bg-muted transition-colors"
        >
        ...
        <DialogContent className="bg-background border-border text-foreground sm:max-w-md">
          ...
          <div className="pt-4">
            <input
              ...
              className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-[#00C300] uppercase placeholder:normal-case placeholder:text-muted-foreground"
            />
            ...
            <div className="mt-4 space-y-2">
              ...
              {Object.entries(PROMO_CODES).map(([code, data]) => (
                <div key={code} className="flex items-center justify-between bg-muted rounded-lg px-3 py-2">
              ...
              <div className="flex gap-2">
                {(['GAGA', 'USD'] as CurrencyCode[]).map(c => (
                  <button type="button" key={c}
                    ...
                    className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${depositCurrency === c ? 'bg-[#00C300] text-white' : 'bg-muted text-muted-foreground'
                      }`}
                  >
                    {c}
                  </button>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0.00"
                  className="w-full bg-muted rounded-xl pl-10 pr-4 py-3 text-foreground focus:outline-none focus:ring-2 focus:ring-[#00C300]"
                />
              </div>
            </div>
      ...
      <Dialog open={showWithdraw} onOpenChange={setShowWithdraw}>
        <DialogContent className="bg-background border-border text-foreground sm:max-w-md">
          <DialogHeader><DialogTitle className="text-foreground flex items-center gap-2"><TrendingDown size={18} className="text-[#FF3B30]" /> Withdraw</DialogTitle></DialogHeader>
          <div className="pt-4 space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                {(['GAGA', 'USD'] as CurrencyCode[]).map(c => (
                  <button type="button" key={c}
                    ...
                    className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${withdrawCurrency === c ? 'bg-[#00C300] text-white' : 'bg-muted text-muted-foreground'
                      }`}
                  >
                    {c}
                  </button>
            ...
            <div>
              ...
              <input
                ...
                placeholder="0.00"
                className="w-full bg-muted rounded-xl px-4 py-3 text-foreground focus:outline-none focus:ring-2 focus:ring-[#00C300]"
              />
            </div>
            <div>
      ...
      <Dialog open={showConvert} onOpenChange={setShowConvert}>
        <DialogContent className="bg-background border-border text-foreground sm:max-w-md">
          <DialogHeader><DialogTitle className="text-foreground flex items-center gap-2"><ArrowRightLeft size={18} className="text-[#00C300]" /> Convert Currency</DialogTitle></DialogHeader>
          <div className="pt-4 space-y-4">
            <div className="flex border-b border-border overflow-x-auto">
              {(['GAGA_USD', 'USD_GAGA'] as const).map(pair => (
                <button type="button" key={pair}
                  onClick={() => setConvertTab(pair)}
                  className={`px-3 py-2 text-xs font-medium whitespace-nowrap transition-colors ${convertTab === pair ? 'text-[#00C300] border-b-2 border-[#00C300]' : 'text-muted-foreground'
                    }`}
                >
                  {pair.replace('_', ' → ')}
                </button>
          ...
          <div className="pt-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              {STAKING_TIERS.filter(t => t.minCoins > 0).map(t => (
                <div
                  key={t.label}
                  className={`flex items-center justify-between p-2.5 rounded-lg ${coins >= t.minCoins ? 'bg-[#00C300]/10 border border-[#00C300]/30' : 'bg-muted'
                    }`}
                >
                  <div className="flex items-center gap-2">
                    <Award size={14} className={coins >= t.minCoins ? 'text-[#00C300]' : 'text-muted-foreground'} />
                    <span className={`text-sm ${coins >= t.minCoins ? 'text-[#00C300] font-medium' : 'text-muted-foreground'}`}>{t.label}</span>
                  </div>
                  <div className="text-right">
      ...
      <Dialog open={showSecurity} onOpenChange={setShowSecurity}>
        <DialogContent className="bg-background border-border text-foreground sm:max-w-md">
          <DialogHeader><DialogTitle className="text-foreground flex items-center gap-2"><Shield size={18} className="text-[#00C300]" /> Wallet Security</DialogTitle></DialogHeader>
          <div className="pt-4 space-y-4">
            <div className="bg-muted rounded-2xl p-4 text-center">
            ...
            {!hasPinSet() ? (
              ...
              <>
                <button type="button" onClick={() => { setShowSecurity(false); setPinMode('set'); }}
                  className="w-full bg-muted text-foreground rounded-xl py-3 text-sm font-bold hover:bg-muted transition-colors"
                >
                  Change PIN
                </button>
