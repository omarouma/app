...
export default function GroupInfoPage() {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#F5F5F5] flex flex-col">
      {/* Header */}
      <div className="shrink-0 flex items-center gap-3 px-4 py-3 bg-white border-b border-[#EBEBEB]">
        <button type="button" onClick={() => navigate(-1)} className="p-2 -ml-2 active:bg-gray-100 rounded-full text-[#111111]">
          <ArrowLeft size={24} strokeWidth={1.5} />
        </button>
        <h1 className="text-lg font-bold text-[#111111]">Group Info</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide pb-8">
          ...
          {editingName ? (
            <div className="flex items-center gap-2 w-full max-w-xs">
              <input
                ...
                className="flex-1 text-center text-lg font-bold text-[#111111] border-b border-[#00C300] focus:outline-none pb-1"
              />
              <button type="button" onClick={handleSaveName} disabled={busy} className="text-[#00C300]"><Check size={20} /></button>
              <button type="button" onClick={() => { setEditingName(false); setNameDraft(group.name || ''); }} className="text-[#8D8D8D]"><X size={20} /></button>
            </div>
          ) : (
            <button type="button" disabled={!isAdmin} onClick={() => isAdmin && setEditingName(true)} className="flex items-center gap-2">
              <span className="text-lg font-bold text-[#111111]">{group.name}</span>
              {isAdmin && <Pencil size={14} className="text-[#8D8D8D]" />}
            </button>
          )}
          <p className="text-xs text-[#8D8D8D]">{members.length} members</p>
        </div>

        {/* Description */}
        <div className="bg-white mt-2 px-4 py-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs font-medium text-[#8D8D8D] uppercase tracking-wide">Description</span>
            {isAdmin && !editingDesc && (
          ...
          {editingDesc ? (
            <div className="flex items-start gap-2">
              <textarea
                ...
                className="flex-1 text-sm text-[#111111] bg-[#F5F5F5] rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#00C300] resize-none"
              />
              <button type="button" onClick={handleSaveDesc} disabled={busy} className="text-[#00C300] mt-1"><Check size={20} /></button>
              <button type="button" onClick={() => { setEditingDesc(false); setDescDraft(group.description || ''); }} className="text-[#8D8D8D] mt-1"><X size={20} /></button>
            </div>
          ) : (
            <p className="text-sm text-[#111111]">{group.description || <span className="text-[#8D8D8D]">No description</span>}</p>
          )}
        </div>

        {/* Quick actions */}
        <div className="bg-white mt-2 divide-y divide-[#EBEBEB]">
          <button type="button" onClick={handleToggleMute} className="w-full flex items-center gap-3 px-4 py-3.5 active:bg-gray-50">
            {group.isMuted ? <VolumeX size={20} className="text-[#8D8D8D]" /> : <Volume2 size={20} className="text-[#8D8D8D]" />}
            <span className="flex-1 text-left text-sm text-[#111111]">{group.isMuted ? 'Unmute notifications' : 'Mute notifications'}</span>
          </button>
          <button type="button" onClick={() => setShowInviteModal(true)} className="w-full flex items-center gap-3 px-4 py-3.5 active:bg-gray-50">
            <Link2 size={20} className="text-[#8D8D8D]" />
            <span className="flex-1 text-left text-sm text-[#111111]">Invite via link</span>
          </button>
        </div>
        ...
        {isAdmin && (
          <div className="bg-white mt-2">
            <div className="px-4 pt-3 pb-1 text-xs font-medium text-[#8D8D8D] uppercase tracking-wide">Admin Controls</div>
            <button type="button" onClick={() => handleToggleSetting('onlyAdminsCanPost')} className="w-full flex items-center gap-3 px-4 py-3.5 active:bg-gray-50">
              {group.settings?.onlyAdminsCanPost ? <Lock size={20} className="text-[#00C300]" /> : <Unlock size={20} className="text-[#8D8D8D]" />}
              <span className="flex-1 text-left text-sm text-[#111111]">Only admins can send messages</span>
              <span className={`w-11 h-6 rounded-full transition-colors relative ${group.settings?.onlyAdminsCanPost ? 'bg-[#00C300]' : 'bg-[#E5E5EA]'}`}>
                <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all ${group.settings?.onlyAdminsCanPost ? 'left-[22px]' : 'left-0.5'}`} />
              </span>
            </button>
            <button type="button" onClick={() => handleToggleSetting('onlyAdminsCanAdd')} className="w-full flex items-center gap-3 px-4 py-3.5 active:bg-gray-50 border-t border-[#EBEBEB]">
              {group.settings?.onlyAdminsCanAdd ? <Lock size={20} className="text-[#00C300]" /> : <Unlock size={20} className="text-[#8D8D8D]" />}
              <span className="flex-1 text-left text-sm text-[#111111]">Only admins can add members</span>
              <span className={`w-11 h-6 rounded-full transition-colors relative ${group.settings?.onlyAdminsCanAdd ? 'bg-[#00C300]' : 'bg-[#E5E5EA]'}`}>
                <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all ${group.settings?.onlyAdminsCanAdd ? 'left-[22px]' : 'left-0.5'}`} />
              </span>
            </button>
          </div>
        )}
        ...
        <div className="bg-white mt-2">
          <div className="flex items-center justify-between px-4 pt-3 pb-2">
            <span className="text-xs font-medium text-[#8D8D8D] uppercase tracking-wide">{members.length} Members</span>
            {isAdmin && (
          ...
          <div className="px-4 pb-2">
            <div className="flex items-center gap-2 bg-[#F5F5F5] rounded-xl px-3 py-2">
              ...
              <input
                ...
                className="flex-1 bg-transparent text-sm text-[#111111] focus:outline-none placeholder:text-[#8D8D8D]"
              />
            </div>
          </div>
          {filteredMembers.map((m) => (
            <div key={m.id} className="flex items-center gap-3 px-4 py-3 border-t border-[#EBEBEB]">
              <img
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[#111111] truncate flex items-center gap-1.5">
                  {m.name}
                  {m.isCreator && <Crown size={13} className="text-[#FFB300]" />}
                  {m.isAdmin && !m.isCreator && <Shield size={13} className="text-[#00C300]" />}
                </p>
                {m.username && <p className="text-xs text-[#8D8D8D] truncate">@{m.username}</p>}
              </div>
              {isAdmin && m.id !== currentUser?.id && (
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className="p-2 rounded-full hover:bg-gray-100 text-[#8D8D8D]"
          ...
          {filteredMembers.length === 0 && (
            <p className="px-4 py-6 text-center text-sm text-[#8D8D8D]">No members match your search.</p>
          )}
        </div>

        {/* Danger zone */}
        <div className="bg-white mt-2">
          <button type="button" onClick={() => setShowLeaveConfirm(true)} className="w-full flex items-center gap-3 px-4 py-3.5 active:bg-gray-50 text-[#FF3B30]">
            <LogOut size={20} />
        ...
        {showAddMembers && (
          ...
          >
            <motion.div
              ...
              className="w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl max-h-[80dvh] flex flex-col"
              onClick={e => e.stopPropagation()}
            >
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#EBEBEB]">
                <h3 className="text-base font-bold text-[#111111]">Add Members</h3>
                <button type="button" onClick={() => setShowAddMembers(false)} className="p-1 rounded-full hover:bg-gray-100 text-[#8D8D8D]"><X size={20} /></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {addableFriends.length === 0 ? (
                  <p className="px-4 py-8 text-center text-sm text-[#8D8D8D]">No friends available to add.</p>
                ) : addableFriends.map(f => (
                  <button
                    key={f.id}
                    type="button"
                    onClick={() => handleAddMember(f.id)}
                    className="w-full flex items-center gap-3 px-4 py-3 active:bg-gray-50 border-b border-[#EBEBEB]"
                  >
                    <img src={sanitizeMediaUrl(f.avatar) || getDefaultAvatar(f.id)} className="w-10 h-10 rounded-full object-cover" alt={f.name} />
                    <div className="flex-1 text-left min-w-0">
                      <p className="text-sm font-medium text-[#111111] truncate">{f.name}</p>
                      {f.username && <p className="text-xs text-[#8D8D8D] truncate">@{f.username}</p>}
                    </div>
        ...
        {showInviteModal && (
          ...
          >
            <motion.div
              ...
              onClick={e => e.stopPropagation()}
            >
              <h3 className="text-base font-bold text-[#111111] mb-1">Invite via link</h3>
              <p className="text-xs text-[#8D8D8D] mb-4">Anyone with this link can join the group.</p>
              {inviteCode ? (
                <>
                  <div className="flex items-center gap-2 bg-[#F5F5F5] rounded-xl px-3 py-2.5 mb-3">
                    <Link2 size={16} className="text-[#8D8D8D] shrink-0" />
                    <span className="flex-1 text-xs text-[#111111] truncate">{inviteUrl}</span>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={handleShareInvite} className="flex-1 flex items-center justify-center gap-2 bg-[#F5F5F5] text-[#111111] rounded-xl py-2.5 text-sm font-medium">
                      Share
                    </button>
                  </div>
                  {isAdmin && (
        ...
        {showLeaveConfirm && (
          ...
          >
            <motion.div
              ...
              onClick={e => e.stopPropagation()}
            >
              <h3 className="text-base font-bold text-[#111111] mb-1">Leave group?</h3>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowLeaveConfirm(false)} className="flex-1 bg-[#F5F5F5] text-[#111111] rounded-xl py-2.5 text-sm font-medium">Cancel</button>
