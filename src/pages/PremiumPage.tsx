    ...
    <div className="min-h-[100dvh] bg-secondary pb-20">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-8">
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-primary to-[#00A300] rounded-2xl p-6 text-white relative overflow-hidden"
        >
          ...
          {PREMIUM_PLANS.map((plan) => {
            ...
            return (
              <motion.div
                ...
                className={`relative bg-card rounded-2xl border-2 p-5 transition-colors ${
                  isCurrent
                    ? 'border-primary'
                    : isPopular
                    ? 'border-primary/50'
                ...
                <button type="button" onClick={() => handleUpgrade(plan.id)}
                  ...
                  className={`w-full py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                    isCurrent
                      ...
                      : isPopular
                      ? 'bg-primary hover:bg-[#00A300] text-white active:scale-95'
        ...
        <div className="bg-card rounded-2xl p-5 shadow-sm">
          ...
          <div className="bg-secondary rounded-xl p-4 flex items-center gap-3 mb-4">
            ...
            <button type="button" onClick={handleCopyReferral}
              className="p-2.5 bg-card rounded-xl hover:bg-primary/10 transition-colors border border-border"
              title="Copy code"
            >
              <Copy size={18} className={copied ? 'text-primary' : 'text-muted-foreground'} />
            </button>
            <button type="button" onClick={handleShareReferral}
              className="p-2.5 bg-primary rounded-xl hover:bg-[#00A300] transition-colors"
              title="Share"
            >
          ...
          <button type="button" onClick={() => setShowReferral(!showReferral)}
            className="w-full py-2.5 rounded-xl border border-border text-sm font-medium text-foreground hover:bg-secondary transition-colors"
          >
                ...
                <div className="pt-4 space-y-3">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-secondary rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary placeholder:text-muted-foreground"
                    />
                    <button type="button" onClick={handleApplyReferral}
                      disabled={applyingReferral || !referralInput.trim()}
                      className="bg-primary hover:bg-[#00A300] text-white px-4 py-2 rounded-xl text-sm font-bold transition-colors disabled:opacity-50"
                    >
        ...
        {upgrading && activePlan && (
          ...
          >
            <motion.div
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
              className="bg-card rounded-2xl p-6 mx-4 max-w-xs w-full text-center"
            >
      ...
      {[0, 1, 2].map((i) => (
        <div
          ...
          className={`w-4 h-4 rounded-full border border-white ${
            i < count ? 'bg-primary' : 'bg-muted-foreground'
          }`}
        />
      ))}
    </div>
