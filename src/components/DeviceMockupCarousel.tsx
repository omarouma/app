...
export default function DeviceMockupCarousel() {
  ...
  return (
    <div className="relative">
      ...
      {/* Phone Frame */}
      <div className="w-[220px] h-[440px] sm:w-[260px] sm:h-[520px] bg-card rounded-[2rem] sm:rounded-[2.5rem] border-4 border-border shadow-2xl relative overflow-hidden mx-auto">
        {/* Status bar */}
        <div className="absolute top-0 left-0 right-0 h-8 bg-white/80 backdrop-blur-sm z-10 flex items-center justify-between px-5 pt-1">
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-20 h-5 bg-secondary rounded-b-xl" />
        ...
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-2 border-b border-border/60">
              ...
              <span className="text-[11px] font-bold text-foreground">{slide.title}</span>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-4 overflow-hidden">
                ...
                <div className="w-full space-y-2">
                  {slide.preview.map((msg, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.12 }}
                      ...
                      <div className={`max-w-[78%] px-3 py-2 rounded-2xl ${msg.me ? 'rounded-br-sm text-white' : 'rounded-bl-sm bg-secondary text-foreground'}`}
              ...
              {slide.isCall && (
                <div className="w-full text-center">
                  ...
                  <p className="text-[12px] font-bold text-foreground mb-1">Kamal Hossain</p>
                ...
                <div className="w-full">
                  ...
                  <div className="mt-2 bg-secondary rounded-lg p-2">
                    ...
                    <div className="flex justify-between items-center">
                      <p className="text-[9px] text-foreground">Tip from @fan123</p>
              ...
              {slide.isVoice && (
                <div className="w-full text-center">
                  <p className="text-[10px] font-bold text-foreground mb-3">🎙️ Tech Talk BD</p>
                  ...
                  <p className="text-[9px] text-muted-foreground mb-2">4 speakers • 128 listeners</p>
                ...
                <div className="w-full">
                  <div className="grid grid-cols-2 gap-1.5">
                    {[
                      ...
                      <div key={item.name} className="bg-secondary rounded-xl p-2 text-center">
                        <p className="text-xl mb-1">{item.emoji}</p>
                        <p className="text-[9px] font-medium text-foreground leading-tight">{item.name}</p>
      ...
      <button type="button" onClick={() => goTo((current - 1 + slides.length) % slides.length)}
        className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-5 w-8 h-8 rounded-full bg-card border border-border flex items-center justify-center text-muted-foreground hover:text-foreground shadow-md">
        <ChevronLeft size={16} />
      </button>
      <button type="button" onClick={() => goTo((current + 1) % slides.length)}
        className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-5 w-8 h-8 rounded-full bg-card border border-border flex items-center justify-center text-muted-foreground hover:text-foreground shadow-md">
