...
export function InputBar({
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {replyingTo && (
          <motion.div
            ...
            animate={{ height: 'auto' }}
            exit={{ height: 0 }}
            className="shrink-0 bg-card border-t border-border px-4 py-2 flex items-center gap-2"
          >
      ...
      <AnimatePresence>
        {showAttachments && (
          <motion.div
            ...
            animate={{ height: 220 }}
            exit={{ height: 0 }}
            className="shrink-0 bg-secondary border-t border-border overflow-hidden z-10"
          >
      ...
      {/* Input */}
      <div className="shrink-0 bg-secondary px-3 py-2.5 flex items-end gap-3 z-20">
        <button
          type="button"
          onClick={onToggleAttachments}
          className={`p-1.5 mb-0.5 rounded-full transition-colors ${showAttachments ? 'bg-accent text-foreground' : 'text-muted-foreground hover:bg-accent'}`}
...
{isRecording ? (
          <div className="flex-1 bg-card rounded-2xl border border-destructive flex items-center px-4 min-h-[40px] gap-3">
            <div className="w-3 h-3 rounded-full bg-destructive animate-pulse shrink-0" />
            <span className="text-destructive text-sm font-medium shrink-0">{Math.floor(duration / 60)}:{(duration % 60).toString().padStart(2, '0')}</span>
          ...
          <div className="flex-1 bg-card rounded-2xl border border-border flex items-center px-3 min-h-[40px] max-h-[128px]">
            <textarea
              ref={inputRef}
              value={input}
              rows={1}
            ...
            />
            <button type="button" className={`p-1 transition-colors ${showStickerPicker ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => { setShowStickerPicker(p => !p); if (showEmojiPicker) onToggleEmojiPicker(); }} aria-label="Open sticker picker">
              <Sticker size={20} strokeWidth={1.5} />
            </button>
            <button type="button" className={`p-1 transition-colors mx-1 ${showEmojiPicker ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => { onToggleEmojiPicker(); setShowStickerPicker(false); }} aria-label="Open emoji picker">
              <Smile size={20} strokeWidth={1.5} />
            </button>
            {!input.trim() && (
              <button type="button" className="text-muted-foreground p-1 hover:text-foreground transition-colors" onClick={onStartRecording} aria-label="Toggle voice recording">
        ...
        {isRecording ? (
          <button type="button" onClick={onVoiceSend} className="mb-1 p-1.5 text-white bg-destructive rounded-full active:scale-95 transition-transform shadow-sm" aria-label="Send voice message">
            <Send size={18} />
          </button>
        ) : input.trim() ? (
          <div className="flex items-center gap-1">
            <button type="button" onClick={onSchedule} className="mb-0.5 p-1.5 text-muted-foreground hover:bg-accent rounded-full transition-colors" title="Schedule message">
              <Clock size={22} strokeWidth={1.5} />
            </button>
            <button type="button" onClick={onSend} className="mb-1 p-1.5 text-primary-foreground bg-primary rounded-full active:scale-95 transition-transform shadow-sm" aria-label="Send message">
              <Send size={18} />
            </button>
          </div>
        ) : (
          <button type="button" className="mb-0.5 p-1.5 text-muted-foreground hover:bg-accent rounded-full transition-colors" onClick={onSchedule} aria-label="Schedule message">
      ...
      <AnimatePresence>
        {showEmojiPicker && (
          <motion.div
            ...
            className="shrink-0 z-20 bg-card border-t border-border px-3 py-2 overflow-hidden"
          >
