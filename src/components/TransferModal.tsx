  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed top-0 right-0 bottom-0 left-0 bg-black/50 z-50 flex items-end sm:items-center justify-center"
        onClick={onClose}
      >
        <motion.div
          initial={{ y: '100%' }}
          ...
          className="bg-background w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl overflow-hidden"
          ...
          <div className="flex border-b border-border">
            <button type="button" onClick={() => { setTab('coins'); setError(''); setSuccess(''); }}
              className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                tab === 'coins' ? 'text-[#00C300] border-b-2 border-[#00C300]' : 'text-muted-foreground'
              }`}
            >
            ...
            <button type="button" onClick={() => { setTab('usd'); setError(''); setSuccess(''); }}
              className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                tab === 'usd' ? 'text-[#00C300] border-b-2 border-[#00C300]' : 'text-muted-foreground'
              }`}
            >
