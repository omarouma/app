      ...
      <div className="p-4 space-y-4">
        {[1, 2, 3].map(i => (
          <div key={i} className={`flex ${i % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[60%] ${i % 2 === 0 ? 'bg-primary/20' : 'bg-secondary'} rounded-2xl px-4 py-3 animate-pulse`}>
  ...
  return (
    <div className="p-4 space-y-3">
      {Array.from({ length: count }).map((_, i) => (
        ...
        >
          <div className={`${variant === 'list' ? 'w-14 h-14 rounded-2xl' : 'w-10 h-10 rounded-full'} bg-secondary shrink-0`} />
