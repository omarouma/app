...
export function getIceServers(): RTCIceServer[] {
  // Legacy live/voice-room clients still need migration to authenticated ICE.
  // No public relays, provider credentials, or paid-service fallbacks.
  if (env.DEV && env.VITE_TURN_SERVER_URL && env.VITE_TURN_SERVER_USERNAME && env.VITE_TURN_SERVER_CREDENTIAL) {
    return [{ urls: env.VITE_TURN_SERVER_URL, username: env.VITE_TURN_SERVER_USERNAME, credential: env.VITE_TURN_SERVER_CREDENTIAL }];
  }
  return [];
}
